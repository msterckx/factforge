...
// Build a lookup from id → imageUrl so the config file drives the images.
// To fix a missing image, edit src/config/caesarImages.json.
const imageMap = new Map(imageConfig.map((e) => [e.id, e.imageUrl]));

const data: Omit<Caesar, "imageUrl">[] = [
  {
    id: 1,
    name: "Julius Caesar",
    reign: "49–44 BC",
