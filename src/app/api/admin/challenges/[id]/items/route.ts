...
export async function POST(req: Request, { params }: Params) {
  ...
  const { position, name, imageUrl, descriptionEn, descriptionNl, dates, fact, hint, achievement } = body;
  ...
  const [item] = db
    ...
    .values({ gameId: Number(id), position, name, imageUrl: imageUrl ?? "", descriptionEn: descriptionEn ?? "", descriptionNl: descriptionNl ?? "", dates, fact, hint, achievement })
    .returning()
    .all();
