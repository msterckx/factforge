...
export async function POST(req: Request, { params }: Params) {
  ...
  const { position, name, imageUrl, descriptionEn, descriptionNl, dates, milestoneEn, milestoneNl, clueEn, clueNl, hint, achievement } = body;
  ...
  const [item] = db
    ...
    .values({ gameId: Number(id), position, name, imageUrl: imageUrl ?? "", descriptionEn: descriptionEn ?? "", descriptionNl: descriptionNl ?? "", dates, milestoneEn, milestoneNl, clueEn, clueNl, hint, achievement })
    .returning()
    .all();
